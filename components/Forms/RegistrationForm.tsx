...
const PatientForm = () => {
  ...
  const fullName: any = sessionStorage.getItem("patientName");
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const res = await axios.post(`${process.env.NEXT_PUBLIC_URL}/api/registration`,
        formData,
        {
          headers: {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className=''>
      ...
      <div className='flex flex-col mb-4'>
        ...
        <div className="my-3 flex justify-start gap-3 items-center">
          <div className='text-white font-sans'>
            Upload Progress
          </div>
          <div className='border border-input-border w-[100px]'>
            <div className={`h-[2px] bg-white transition-all ease-out`} style={{ width: `${progress}px` }}>
            </div>
          </div>
        </div>
